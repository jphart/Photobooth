...
exports.save = (picturesPath, prefix, contents, done) => {
  const base64Data = contents.replace(/^data:image\/jpeg;base64,/, '')
  const date = new Date();
  const imgPath = path.join(picturesPath, `${prefix}-${date.getFullYear()}-${date.getMonth() +1}-${date.getDate()} ${date.getHours()}-${date.getMinutes()}-${date.getSeconds()}-${date.getMilliseconds()}.jpg`)
